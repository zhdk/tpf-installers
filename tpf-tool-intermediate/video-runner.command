...
##############################################################################
# 
# the video-runner.command
# 
# The video-runner is a part of the tpf-tools and starts 
# the pre-configured sessions for usb- & facetime-camera.
#
# © 2018-2021 Johannes Schuett and the TPF-Team @ Zurich University of the Arts
# 
##############################################################################
echo "‘tpf-video-runner.command’ opens all necessary video tools for your telematic session."
echo "©2021 Johannes Schuett and TPF-Team @ Zurich University of the Arts"
echo "------------------------------------------------------------"
echo "Detailed information can be found here: "
echo -e "\033[1;34m https://networkperformance.space (TPF-Tools)"
echo -e "\033[1;32m ------------------------------------------------------------ \033[0m"
echo -e "\033[1;32m'tpf-video-runner.command' executes and supports the following steps:\033[0m"
echo -e "\033[1;33m [1/1] Choosing the camera device.\033[0m"
echo -e "\033[1;33m [1/2] Choosing the bitsize.\033[0m"
echo -e "\033[1;33m [1/3] Choosing the mode.\033[0m"
echo -e "\033[1;33m [1/4] Choosing the port.\033[0m"
echo -e "\033[1;33m [1/5] Choosing the address of the remote partner and making a connection.\033[0m"
echo "-------------------------------------------------------------"
...
echo -e "\033[1;32m What is your camera device? \033[0m"
...
echo -e "\033[1;31m [1/2]What is the desired bitsize? \033[0m"
...
while true; do
 ...
 input="$bitsize"
echo -e "\033[1;32m Your choosen camera bitsize is libavcodec:codec=H.264:bitrate=$bitsize \033[0m"


echo -e "\033[1;31m [1/3] What is your desired mode? \033[0m"
...
while true; do
 ...
 input="$mode" 
echo -e "\033[1;32m Your choosen mode for the camera avfoundation:=device$:$mode \033[0m"


echo -e "\033[1;31m [1/4] Sometimes the server ports are busy and we have to switch to another port: \033[0m"
...
echo -e "\033[1;31m [1/5] What is the address of your remote partner? \033[0m"
...
while true; do
    read -p "Choose your port by writing the appropriate number:     (127.0.0.1):         " server
    [[ $answer == "y" || $answer == "Y" || $answer == "" ]] && break
    [[ $answer == "n" || $answer == "N" ]] && exit 0
done
input="$server"
...
/Applications/uv-qt.app/Contents/MacOS/uv --capture-filter preview -t avfoundation:device=$device:mode=$mode:fps=30:preset=high -c libavcodec:codec=H.264:bitrate=$bitsize -d multiplier:gl#preview -f rs:200:220 -P $port  $server 
